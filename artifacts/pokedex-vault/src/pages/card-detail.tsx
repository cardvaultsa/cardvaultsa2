...
export default function CardDetail() {
  ...
  const [editForm, setEditForm] = useState<CardUpdate>({});
  ...
  const { data: images } = useListCardImages(cardId, {
    query: { enabled: !!cardId, queryKey: getListCardImagesQueryKey(cardId) },
  });
  ...
  const addImage = useAddCardImage({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListCardImagesQueryKey(cardId) });
        queryClient.invalidateQueries({ queryKey: getGetCardQueryKey(cardId) });
      },
    },
  });

  const deleteImage = useDeleteCardImage({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListCardImagesQueryKey(cardId) });
      },
  ...
  const realizedProfit = isSold && card.purchasePrice != null && card.soldPrice != null
    ? card.soldPrice - card.purchasePrice
    : null;
  const unrealizedProfit = !isSold && card.purchasePrice != null && card.marketValue != null
    ? card.marketValue - card.purchasePrice
    : null;
  ...
  const startEdit = () => {
    setEditForm({
      ...
      marketValue: card.marketValue ?? undefined,
      askingPrice: card.askingPrice ?? undefined,
      soldPrice: card.soldPrice ?? undefined,
      soldDate: card.soldDate ?? undefined,
      purchaseDate: card.purchaseDate ?? undefined,
      purchaseSource: card.purchaseSource ?? undefined,
      tradeNotes: card.tradeNotes ?? undefined,
      status: card.status as CardUpdate["status"],
  ...
  const setField = (k: keyof CardUpdate, v: string | number | undefined) =>
    setEditForm((f) => ({ ...f, [k]: v }));
    ...
    <Layout>
      <div className="max-w-2xl mx-auto space-y-5">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold flex-1 min-w-0 truncate">{card.name}</h1>
          ...
          <button
            onClick={() => {
              if (confirm("Delete this card?")) deleteCard.mutate({ id: cardId });
            }}
        ...
        <div className="bg-card border border-card-border rounded-lg divide-y divide-border">
          {[
            ...
            ["Grade", card.grade ? `${card.grader ?? ""} ${card.grade}`.trim() : null],
            ["Quantity", card.quantity],
            ["Location", card.location],
            ["Purchase Price", fmt(card.purchasePrice)],
            ["Purchase Date", card.purchaseDate],
            ["Purchase Source", card.purchaseSource],
            ["Market Value", fmt(card.marketValue)],
            ["Asking Price", fmt(card.askingPrice)],
            ...(isSold ? [["Sold Price", fmt(card.soldPrice)], ["Sold Date", card.soldDate]] : []),
          ]
            .filter(([, v]) => v != null && v !== "")
            .map(([label, value]) => (
          ...
          {card.tradeNotes && (
            <div className="px-4 py-2.5">
              ...
              <div className="text-sm">{card.tradeNotes}</div>
            </div>
          )}
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => generateListing.mutate({ data: { itemType: "card", itemId: cardId } })}
        ...
        <div>
          ...
          <ImageGallery
            images={images ?? []}
            onDelete={(imageId) => deleteImage.mutate({ id: cardId, imageId })}
          />
        </div>
      </div>
              ...
              onPhotosUploaded={async (photos) => {
                for (const photo of photos) {
                  ...
                  if (!card.coverImagePath) {
                    updateCard.mutate({ id: cardId, data: { coverImagePath: photo.objectPath } });
                  }
                }
              }}
        ...
        <div className="fixed inset-0 z-40 bg-black/50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-background w-full sm:max-w-lg rounded-t-2xl sm:rounded-xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-background border-b border-border px-5 py-4">
              <h2 className="font-semibold text-lg">Edit Card</h2>
            </div>
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Name", key: "name" as const, span: 2, placeholder: "" },
                  { label: "Set", key: "set" as const, placeholder: "" },
                  { label: "Card #", key: "cardNumber" as const, placeholder: "" },
                  { label: "Rarity", key: "rarity" as const, placeholder: "" },
                  { label: "Grade", key: "grade" as const, placeholder: "" },
                  { label: "Grader", key: "grader" as const, placeholder: "" },
                  { label: "Location", key: "location" as const, placeholder: "" },
                  { label: "Purchase Source", key: "purchaseSource" as const, placeholder: "eBay, local shop..." },
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setField("status", e.target.value as CardUpdate["status"])}
                  >
            ...
            <div className="sticky bottom-0 bg-background border-t border-border px-5 py-4 flex gap-2">
              ...
              <button
                onClick={() => updateCard.mutate({ id: cardId, data: editForm })}
